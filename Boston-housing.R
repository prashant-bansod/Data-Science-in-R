...
#Outlier plots
par(mfrow=c(2,7)) 
list<-names(prices) #Storing the names of the dataset in a list format
list<-list[-4]
for(i in 1:length(list)) #Plotting the boxplots of all variables and shortlisting which variables need outlier treatment.
{
  boxplot(prices[,list[i]],main=list[i])
}

#Restoring the par parameters to normal
dev.off()

# replacing the outlier values by the mean values
#Outlier treatment
for(i in 1:length(list)) ##For loop to replace all the outlier values with the mean value
{
...
#Studying the histogram of the DV and the transformed histogram
hist(prices$MEDV)
...
#Log transformations
#Creating the log transformation for all variables
...
#Reg2 : removing the insignificant values
...
#Reg3 _ removing insignificant values
...
# there seems to be an inverse pattern for some points
